...
Let Jeremy try a complete but manageable task solo.

### Task

Load the test dataset, run prediction on 1 test image, and plot it.
...
pred = model(test_img)
print("Predicted label:", pred.argmax(dim=1).item())

plt.imshow(test_img[0].squeeze(), cmap='gray')
plt.title(f"Actual Label: {test_label}")
plt.show()
```

Encourage him to try 2-3 different test images.
